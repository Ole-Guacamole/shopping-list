...
router.get(
  "/shopping-lists/:listId/shared-users",
  async (req: Request, res: Response, next: NextFunction) => {
    const { listId } = req.params;
    try {
      ...
      console.error(
        `Error fetching users for shopping list with ID ${listId}:`,
        error
      );
      next(error); // Pass the error to the error-handling middleware
    }
...
router.delete(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      console.log(
        `Deleting user shopping list links for shopping list with ID ${listId}`
      );
      ...
      console.log(
        `Deleted user shopping list links for shopping list with ID ${listId}`
      );
